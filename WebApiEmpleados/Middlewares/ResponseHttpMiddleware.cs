namespace WebApiEmpleados.Middlewares
{
    ...
    public class ResponseHttpMiddleware
    {
        ...
        //Invoke o InvokeAsync si es asincrono, debe retornar un Task
        public async Task InvokeAsync(HttpContext context)
        {
            using(var ms = new MemoryStream())
            {

                //se asigna el body del response en una varianle y se le da el valor de memorystream
                var bodyOriginal = context.Response.Body;
                context.Response.Body = ms;

                //Para permitir continuar con la linea
                await siguiente(context);

                //guardar lo que le respondemos al cliente en el string
                ms.Seek(0, SeekOrigin.Begin);
                string response = new StreamReader(ms).ReadToEnd();
                ms.Seek(0, SeekOrigin.Begin);

                //Leeremos el stream y lo colocamos como estaba
